...
import WebcamSim from "@/components/web.cam.sim";
import TextHighlight from "@/components/text.highlight";
import VoiceNoteSim from "@/components/voice.note.sim";
...
const BacaPage = () => {
  ...
  const calculateProgress = () => {
    ...
    const wordsReadSoFar = storyData.sentences.slice(0, currentSentenceIdx).join(" ").split(" ").length + currentWordIdx;
  ...
  return (
    <main className="open-dyslexic min-h-screen p-4" style={{ backgroundColor: colors.background }}>
      <div className="text-center mb-6">
        ...
        <div className="mt-4 bg-gray-200 rounded-full h-3 max-w-md mx-auto">
          <div className="bg-gradient-to-r from-green-400 to-blue-500 h-full rounded-full transition-all duration-300" style={{ width: `${calculateProgress()}%` }}></div>
        </div>
        <p className="text-sm mt-2" style={{ color: colors.secondaryText }}>
          Kata: <strong>{getCurrentWord()}</strong> | Progress: {Math.round(calculateProgress())}% | Distraksi: {readingStats.distractionCount}
        </p>
      </div>
        ...
        <div className="flex flex-col gap-4 mb-6">
          ...
          <div className="bg-white rounded-lg shadow p-4">
            <div className="flex items-center justify-center gap-4">
              ...
              <input
                ...
                max="2000"
                step="100"
                value={2000 - (autoPlaySpeed - 300)}
                onChange={(e) => {
                  const reversedValue = 2000 - (parseInt(e.target.value) - 300);
                  setAutoPlaySpeed(reversedValue);
                }}
          ...
          <div className="flex justify-center gap-4">
            <button
              ...
              disabled={isFinished}
              className={`px-6 py-3 rounded-lg font-bold transition ${isAutoPlaying ? "bg-red-500 hover:bg-red-600 text-white" : "bg-green-500 hover:bg-green-600 text-white"} ${isFinished ? "opacity-50 cursor-not-allowed" : ""}`}
            >
      ...
      {isFinished && (
        <div className="text-center">
          <div className="bg-green-100 border-2 border-green-500 rounded-lg p-6 mb-4">
            <h2 className="text-2xl font-bold text-green-800 mb-2">🎉 Selamat! Kamu telah selesai membaca!</h2>
            <p className="text-green-700">Medal yang kamu dapatkan: {calculateMedal() === "gold" ? "🥇 Emas" : calculateMedal() === "silver" ? "🥈 Perak" : "🥉 Perunggu"}</p>
          </div>

          <Link href={`/pertanyaan?story=${encodeURIComponent(storyData.title)}&medal=${calculateMedal()}`}>
            <button className="px-8 py-4 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-bold text-lg transition">📝 Lanjut ke Pertanyaan</button>
          </Link>
        </div>
      )}
      ...
      {/* {showDistraction && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-red-100 border-2 border-red-500 rounded-lg p-6 text-center max-w-md mx-4">
            ...
            <h3 className="text-xl font-bold text-red-800 mb-2">Kamu Terdistraksi!</h3>
            <p className="text-red-700 mb-4">Ayo fokus kembali ke cerita ya! 📖</p>
            <button onClick={continueReading} className="px-6 py-3 bg-green-500 hover:bg-green-600 text-white rounded-lg font-bold transition">
              ✅ Lanjutkan Membaca
            </button>
          </div>
        </div>
